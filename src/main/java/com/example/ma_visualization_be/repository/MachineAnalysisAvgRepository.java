...
public class MachineAnalysisAvgRepository {
    ...
    private String buildDynamicQuery(String divisionValues) {
        ...
        return """
                ...
                DECLARE @toDate VARCHAR(8) = FORMAT(CASE
                         ...
                         ELSE @EOMonth
                        END, 'yyyyMMdd')
                DECLARE @fromDateTime DATETIME = CONVERT(DATETIME, @fromDate, 112);
                DECLARE @toDateTime DATETIME = DATEADD(SECOND,-1,DATEADD(DAY,1,CONVERT(DATETIME,@toDate,112)));
                ...
                SELECT
                 ...
                 'MovAve' + CAST(n+1 as nvarchar(2)) as MonthDescribe
                INTO #MonthTB
                ...
                INTO #StopHour_Ave
                FROM #StopHour_ORG
                WHERE ((SENDTIME BETWEEN @fromDateTime AND @toDateTime) OR ((ISSUESTATUS = 'WAIT' OR ISSUESTATUS = 'START') AND SENDTIME < @fromD))
                ...
                SELECT Machines.Div, Machines.MacName, m.MonthDescribe, m.StartDT, m.EndDT, Format(m.StartDT,'MMM') + '-' +  Format(m.EndDT,'MMM') as Rank1
                INTO #MonthMachine
                FROM #MonthTB m
                CROSS JOIN (
                ...
                BEGIN
                 ...
                 SELECT 'AVE' as Scale, 'KVH' as Div, topUse.STT as [Rank], topUse.MacName,
                ...
                BEGIN
                 ...
                 SELECT 'AVE' as Scale, topUse.DIV, CAST(topUse.STT AS nvarchar(10)) as [Rank], topUse.MacName,
                  topUse.Act as RepairFee, stpHour.StopHour,
                  topUse.CountMac, 
                  topUse.Ave_RepairFee, 
                  stpHour.StopCase,
                  stpHour.StopHour/stpHour.StopCase as Ave_StopHour,
                  CAST(stpHour.StopCase as Decimal(6,1))/@mn as Ave_StopCase
                 FROM topUse
                 ...
                 SELECT mm.MonthDescribe as Scale, mm.Div, mm.Rank1, mm.MacName,
                  rpf.ACT, stp.StopHour,
                  rpf.CountRP, --CountMac
                  rpf.AveRP, --repair fee ave
                  stp.StopCase,--StopCase
                  stp.AveStp, --Ave_StopHour
                  CAST(stp.StopCase as Decimal(6,1))/3 as Ave_StopCase
                 ...
                 AND rpf.Div IN (SELECT value FROM @div)
                 ORDER BY Div, Scale, [Rank]
                END
                """;
    }
    ...
    private static class MachineAnalysisRowMapper implements RowMapper<MachineAnalysisAvgResponse> {
        ...
        public MachineAnalysisAvgResponse mapRow(ResultSet rs, int rowNum) throws SQLException {
            ...
            response.setRank(rankObj != null ? rs.getString("Rank") : null);
...
//            response.setAveRepairFee(rs.getBigDecimal("Ave_RepairFee"));
            ...
            try {
                response.setStopCase(rs.getObject("Ave_StopCase", Integer.class));
