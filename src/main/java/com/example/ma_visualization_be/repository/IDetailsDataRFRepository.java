...
public interface IDetailsDataRFRepository extends JpaRepository<DummyEntity, Long> {
	...
	@Query(value = """
			...
			""", nativeQuery = true)
	List<IDetailsDataRFDTO> getDailyDetailsRepairFeeByMonths1(
	...
	@Query(value = """
			SELECT 
			 ...
			 SGTXT as Cate,
			 iss.MATNR,
			 COALESCE(part.MAKTX, spot.MAKTX) as MAKTX,
			 CONVERT(DATE,BLDAT,112) as UseDate,
			 KOSTL,
			 KONTO,
			 XBLNR2,
			...
			FROM MANUFASPCPD.dbo.MANUFA_F_PD_DTM_ISSUE iss
			LEFT JOIN MANUFASPCPD.dbo.MANUFA_F_PD_GRB_PART part 
			    ON iss.MATNR = part.MATNR
			LEFT JOIN MANUFASPCPD.SST.DTM_SPOT spot 
			    ON iss.MATNR = spot.MATNR
			LEFT JOIN F2Database.dbo.F2_MA_MACHINE_MASTER mac 
			    ON IIF(AUFNR LIKE 'M%',RIGHT(AUFNR,LEN(AUFNR)-1),AUFNR) = mac.CODE
			LEFT JOIN F2Database.dbo.F2_Cost_Center_Share share 
			    ON iss.KOSTL = share.Cost_Center
			LEFT JOIN F2Database.dbo.F2_Cost_Center_Share_Ratio ratio 
			    ON share.Group_Share = ratio.Dept
			    AND CASE
			        WHEN XBLNR2 LIKE '1566-%' THEN 
			         CASE
			          WHEN LEFT(KOSTL,6) IN ('614100','614000') THEN 'PRESS'    
			...
			WHERE KONTO = '570600'
			AND (',' + REPLACE(:month, '-', '') + ',' LIKE '%,' + LEFT(BLDAT,6) + ',%')
			AND BLDAT < CONVERT(varchar,GETDATE(),112)
