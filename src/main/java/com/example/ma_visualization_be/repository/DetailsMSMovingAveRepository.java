...
public class DetailsMSMovingAveRepository {
	...
	private String buildDynamicQuery(String divisionValues) {
		...
		return """
				...
				DECLARE @monthFromDate DATE =
				    CONVERT(DATE, @monthFrom + '01')
				
				DECLARE @monthToDate DATE =
				    IIF(
				        EOMONTH(
				            CONVERT(DATE, @monthTo + '01')
				        ) > GETDATE(),
				...
				SELECT
				    ...
				    CASE
				        ...
				        WHEN mst.DIVISION LIKE '%GUIDE'
				            THEN 'GUIDE'
				
				        WHEN mst.DIVISION LIKE 'SUPPORT%'
				            THEN CASE
				                    ...
				                    ROUND(
				                        (
				                            DATEDIFF(
				                                MINUTE,
				                                ISNULL(CONFIRM_DATE,SENDTIME),
				                                COALESCE(FINISHTIME,@monthToDate)
				                            )
				                        ) /60.0*20/24,
				                        2
				                    )
				...
				INNER JOIN F2Database.dbo.f2_ma_machine_master mst
				...
				LEFT JOIN (
				    ...
				    FROM (
				        ...
				        SELECT
				            ...
				            MIN(STOP_DATE) as Min_Stop,
				
				            MAX(STOP_DATE) as Max_Stop
				...
				LEFT JOIN F2Database.dbo.F2_MA_MACHINE_CONTENT cnt
				    ON dt.REF_NO = cnt.REF_NO
	...
	private String buildDynamicQuery1(String divisionValues) {
		return """
				DECLARE @monthFrom VARCHAR(6) = ?
				DECLARE @monthTo VARCHAR(6) = ?
